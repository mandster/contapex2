...
import { getAllEmployeesFromFirebase, getAllEntriesFromFirebase, getProductByIdFromFirebase } from '../_services/firebaseService';
...
const Calculate = () => {
  ...
  const handleGetRecords = async () => {
    console.log(selectedMonth + " ss " + selectedYear + " cc " + selectedEmployeeId)
    ...
    try {
      ...
      const filteredEntries = allEntries.filter(entry => {
        const entryDate = new Date(entry.dateAdded); // Parse date string into a Date object
        const month = entryDate.getMonth() + 1; // Extract the month from the parsed date
        const year = entryDate.getFullYear(); // Extract the year from the parsed date
        const employeeId = entry.employeeId;
        return month === selectedMonth && year === selectedYear && employeeId === employeeId ;
      });
      // Calculate row total and total month amount
      let total = 0;
      console.log(filteredEntries)
      const updatedEntries = await Promise.all(filteredEntries.map(async entry => {
       // console.log(entry.productId + " dsds " + entry.employeeId)
        const product = await getProductByIdFromFirebase(entry.productId, entry.employeeId);
        const rowTotal = entry.quantity * (product.price || 0); // Use 0 if price is not available
        total += rowTotal;
        return { ...entry, productName: product.productName, price: product.price, rowTotal };
      }));
  ...
  return (
    <div>
      ...
      <div  className="p-2 m-2">
        ...
        <label>Year:</label>
        <select  className="m-2" value={selectedYear} onChange={(e) => setSelectedYear(parseInt(e.target.value))}>
        ...
        <select className="m-2" value={selectedEmployeeId} onChange={(e) => setSelectedEmployeeId(e.target.value)}>
          <option value="">Select Employee</option>
          {employees.map(employee => (
            <option key={employee.id} value={employee.id}>{employee.employeeName}</option>
          ))}
        </select>
      <button className="m-2" onClick={handleGetRecords}>Get Records</button>
      ...
      <ul>
        ...
        {entries.map(entry => (
          <li className="a-item" key={entry.id}>
            ...
            <span className="a-size centered-text">{entry.rowTotal}</span>
          </li>
    ))}
    </ul>
</div>
      <div>
        <strong>Total Month Amount: {totalMonthAmount}</strong>
      </div>
    </div>
  );
};
